...
from action_space import high_velocity_action, transition_action, low_velocity_action
from fish_aggregating_device import fish_aggregating_device
...
class MarinePredatorsAlgorithm(object):
    ...
    def __visualize(self):
        ...
        # 在平均fitness曲线上绘制紫色竖线和五角星标记
        for i, idx in enumerate(self.idx_idx):
            # 获取在 mean_fitness 曲线中对应索引处的 y 值
            y_value = self.mean_fitness[idx]

            # 只在第一次绘制五角星标记时添加图例标签
            if i == 0:
                plt.plot(idx, y_value, marker='*', color='purple', markersize=15,
                         markerfacecolor='white', markeredgewidth=2, label='Phase transition')
            else:
                plt.plot(idx, y_value, marker='*', color='purple', markersize=15,
                         markerfacecolor='white', markeredgewidth=2)
    ...
    def optimizing(self, fitness_function: callable = None, repair_function: callable = None,
        ...
        while self.iter < max_iteration:
            ...
            if self.iter != 0:
                ...
                # 组合进度条显示内容
                status = f'Iteration: {self.iter + 1}/{max_iteration}, {progress_bar} {progress:.2f}% Complete, ' \
            ...
            if current_phase == 1:
                prey_matrix = high_velocity_action(prey_matrix=prey_matrix,
                                                   elite_matrix=elite_matrix,
                                                   P=self.p, device=self.device)
            elif current_phase == 2:
                self.CF = (1 - (self.iter / max_iteration)) ** (2 * (self.iter / max_iteration))
                prey_matrix = transition_action(prey_matrix=prey_matrix,
                                                elite_matrix=elite_matrix,
                                                P=self.p, CF=self.CF, device=self.device)
            elif current_phase == 3:
                self.CF = (1 - (self.iter / max_iteration)) ** (2 * (self.iter / max_iteration))
                prey_matrix = low_velocity_action(prey_matrix=prey_matrix,
                                                  elite_matrix=elite_matrix,
                                                  P=self.p, CF=self.CF, device=self.device)
            ...
            # ##########################################################################################################
            self.CF = (1 - (self.iter / max_iteration)) ** (2 * (self.iter / max_iteration))
            prey_matrix = fish_aggregating_device(prey_matrix=prey_matrix,
                                                  lower_bound=self.lower_bound, upper_bound=self.upper_bound,
                                                  CF=self.CF, FAD=self.FAD, device=self.device)
